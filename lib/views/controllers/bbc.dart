...
class BBCButton extends ButtonControlButton {
  BBCButton(ValueListenable listenable, String name) : super(listenable, name);
          ...
          decoration: BoxDecoration(
            color: () {
              if (button.name.endsWith("R")) {
                return button.isDown() ? Color(0xFF501010) : Color(0xFFF05050);
              }
              if (button.name.endsWith("G")) {
                return button.isDown() ? Color(0xFF501010) : Color(0xFF50F050);
              }
              if (button.name.endsWith("B")) {
                return button.isDown() ? Color(0xFF501010) : Color(0xFF5050F0);
              }
              if (button.name.endsWith("Slowdown")) {
                return button.isDown() ? Color(0xFF300909) : Color(0xFF303030);
              }
              return button.isDown() ? Color(0xFF501010) : Color(0xFF505050);
            }(),
          ),
          child: Icon(() {
            if (button.name.endsWith("+")) return Icons.keyboard_arrow_right;
            if (button.name.endsWith("-")) return Icons.keyboard_arrow_left;
            return null;
          }(), size: areaMin),
        );
      },
            ...
            child: LayoutBuilder(builder: (context, constraints) {
              ...
              return Stack(
                children: [
                  ...
                  Positioned(
                      left:
                          areaWidth * (0.25 / 4 + (0.25 + 0.25 / 4) * 2 - 0.05),
                      top: areaHeight * 0.05,
                      child: Container(
                        width: areaWidth * 0.3,
                        height: areaHeight * 0.3,
